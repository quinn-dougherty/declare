...
;(defun my-retrieve-openai-api-key ()
;  "Retrieve the OpenAI API key from the authinfo file."
;  (require 'auth-source)
;  (let ((creds (auth-source-search :host "api.openai.com"
;                                   :user "apikey"
;                                   :max 1)))
;    (if creds
...
;              (funcall secret)
;            secret))
;      (error "No API key found for api.openai.com in authinfo"))))
;; (use-package! gptel
;; :config
;; (setq! gptel-api-key (funcall 'my-retrieve-openai-api-key)))

;(after! auth-source
;    (setq gptel-api-key (auth-source-user-and-password "apikey"))
; )
...
;; (setq gpt-openai-temperature 0.0001)
#+END_SRC
* Other LLM things
#+BEGIN_SRC elisp
; (setq )
