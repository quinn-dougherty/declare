{ pkgs, pkgs-stable }:
let
  mkShellName = devenvs: builtins.concatStringsSep "-" (map (devenv: devenv.name) devenvs);
  mkShellInputs =
    devenvs:
    (with pkgs; [ nixpkgs-fmt ]) ++ (builtins.concatLists (map (shell: shell.buildInputs) devenvs));
  ...
  combinatorics = rec {
    permute2 = l: builtins.concatMap (x: map (y: if x == y then x else mkDevShell x y) l) l;
    permute3 =
      l:
      with builtins;
      concatMap (
        x:
        let
          perms = permute2 (filter (x': x != x') l);
        in
        map (y: mkDevShell x y) perms
      ) l;
  };
in
{
  developersWithPermutations =
    developers:
    with builtins;
